...
search_result = f"./IPC/{app_name}/{app_name}_store_effect.csv"
pattern = "(.*) [mu]s"
...
## 运行-----------------------------------------
cache_time = 0
cmd = f"./bin/{app_name}/{app_name}_{load}"
...
print("bypass time-------------------------------")
k = 1
for store in range(1, store_require+1):
    print(store,"-----------------------")
    # 设置当前搜索的load
    store_idx = "%d" % store
    os.environ['STORE_IDX'] = store_idx
    os.environ['INJECT_BIT'] = store_idx
    # os.environ['SOURCE_CPP'] = "OFF"
    ...
    # 编译--------------------------------------
    cmd = f"cmake -S . -B build/{app_name}/{store_idx}"
    try:
        subprocess.run(cmd, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        print(f"CMake configuration failed for TARGET_NAME={store_idx}")
        print("Output:\n", e.stdout.decode())

    cmd = f"cmake --build build/{app_name}/{store_idx} -j64"
    try:
        subprocess.run(cmd, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        print("CMake build failed!")
    ...
    # 运行--------------------------------------
    time = 0
    cmd = f"./bin/{app_name}/{app_name}_{store_idx}"
    for i in range(iteration):
        result = subprocess.run(cmd, shell=True, capture_output=True) # 阻塞运行？？
        if result.returncode == 0:
            # print("success\n")
    ...
    with open(search_result, 'a') as f:
        f.write(f'{k},{store_idx},{time},{improve}\n')
    k+=1