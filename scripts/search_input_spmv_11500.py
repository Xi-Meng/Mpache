...
def run():
    ...
    speedup = cache_time/bypass_time
    with open(sssp_brute_result, 'a') as f:
        f.write(f'{cache_time},{bypass_time},{speedup},{improve}\n')
     # k += 1
...
#     cnt += 1
    ...
    # col = row
    # with open(sssp_brute_result, 'a') as f:
    #         f.write(f'row*col: {row}*{col}\n')
    # for spa in sparse:
for spa in np.arange(0.0001, 0.021, 0.0005):
    # print("1")
    ...
    # 生成数据
    cmd = f"python3 /workspace/Inst-level-cache-management/include/{appName}/dataset/data_gen.py  --rows {row} --cols {col} --elements {element}"
