...
def run(numElements):
    ...
    for filename in os.listdir(sssp_dir):
        ...
        cmd = f"{sssp_dir}{filename} {numElements}"
        # print(cmd)
        # print("%s : %s\n" % (k, filename))
        # grbm_count_sum = 0
        time = 0
...
# while(cnt < loop):
'''
_Z12bucketprefixPjS_
_Z13mergeSortPassPK15HIP_vector_typeIfLj4EEPS0_PKiiii
'''
for node in range(20000000, 60000000, 1000000):
# while(cnt < loop):
    cnt += 1
    # node *= 10
    numElements = node
    with open(sssp_brute_result, 'a') as f:
        # f.write(f'稀疏度：{spa}\n')
        f.write(f'numElements: {numElements},')
        print(f"numElements: {numElements}\n")
        # 生成数据
