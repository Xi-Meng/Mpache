...
while(cnt < loop):
    ...
    # for spa in sparse:
    for spa in np.arange(0.016, 0.023, 0.0001):
        element = round(row * col * spa)
        with open(sssp_brute_result, 'a') as f:
            # f.write(f'稀疏度：{spa}\n')
            f.write(f'row*col: {row}*{col}, element: {element}, sparse: {spa},')
        # 生成数据
        cmd = f"python3 /workspace/Inst-level-cache-management/include/jds_sssp/dataset/data_gen.py  --rows {row} --cols {col} --sparse {spa}"
