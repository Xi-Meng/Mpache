...
kernelName = 'performStreamCollide_kernel'
...
for filename in os.listdir(sssp_dir):
    ...
    cmd = f"rocprof -i {metric_path} -o {csv_path_k} {sssp_dir}{filename} -i /workspace/Inst-level-cache-management/include/lbm_main/data/short/input/120_120_150_ldc.of -o /workspace/Inst-level-cache-management/include/lbm_main/data/temp.txt -- 100"
    # print(cmd)
    print("%s : %s\n" % (k, filename))
    ...
    with open(sssp_brute_result, 'a') as f:
        f.write(f'{k},{filename},{aver_1},{aver_2},{aver_3},{aver_4},{aver_5}\n')
    k += 1
