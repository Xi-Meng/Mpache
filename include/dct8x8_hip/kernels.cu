...
void DCT8x8(
    ...
    if (dir == DCT_FORWARD)  {
      hipLaunchKernelGGL(DCT8x8_kernel, dim3(grid), dim3(block), 0, 0, d_Dst, d_Src, stride, imageH, imageW);
    }
    else {
      hipLaunchKernelGGL(IDCT8x8_kernel, dim3(grid), dim3(block), 0, 0, d_Dst, d_Src, stride, imageH, imageW);
    }
}
