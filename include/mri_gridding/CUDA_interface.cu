...
void CUDA_interface (
  ...
  hipMalloc((void**)&sortedSample_d, (n+npad)*sizeof(ReconstructionSample));
...
/*The CUDPP library features highly optimizes implementations for radix sort
  ...
  int* CPUbin;
  hipHostMalloc((void**)&CPUbin,CPUbin_size*sizeof(unsigned int));
  hipMemcpy(CPUbin, idxValue_d+cpuStart, CPUbin_size*sizeof(unsigned int), hipMemcpyDeviceToHost);
