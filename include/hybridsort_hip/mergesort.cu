...
float4* runMergeSort(int listsize, int divisions,
  ...
  hipMalloc((void**)&d_resultList_buff, sizeof(float4)*listsize/4);

  float4 *d_origList_buff;
  hipMalloc((void**)&d_origList_buff, sizeof(float4)*listsize/4);
  hipMemcpyAsync(d_origList_buff, d_origList, sizeof(float4)*listsize/4, hipMemcpyHostToDevice, 0);

  int* d_constStartAddr;
  hipMalloc((void**)&d_constStartAddr, sizeof(int)*(divisions+1));
  hipMemcpyAsync(d_constStartAddr, startaddr, sizeof(int)*(divisions+1), hipMemcpyHostToDevice, 0);
  ...
  hipLaunchKernelGGL(sortElement, dim3(blocks), dim3(THREADS), 0, 0, d_resultList_buff, d_origList_buff, listsize/4);
  ...
  hipMalloc((void**)&d_finalStartAddr, sizeof(unsigned int)*(divisions+1));
  hipMemcpyAsync(d_finalStartAddr, origOffsets,  sizeof(unsigned int)*(divisions+1), hipMemcpyHostToDevice,0);

  unsigned int *d_nullElements;
  hipMalloc((void**)&d_nullElements, sizeof(unsigned int)*divisions);
  hipMemcpyAsync(d_nullElements, nullElements, sizeof(unsigned int)*divisions, hipMemcpyHostToDevice,0);
  ...
  hipLaunchKernelGGL(mergepack, dim3(grids), dim3(threads), 0, 0, 
  ...
  hipMemcpy(d_origList, d_origList_buff, sizeof(float4)*listsize/4, hipMemcpyDeviceToHost);

  hipFree(d_resultList_buff);
  hipFree(d_origList_buff);
  hipFree(d_finalStartAddr);
  hipFree(d_constStartAddr);
  hipFree(d_nullElements);
  free(startaddr);
  return d_origList;
}
