...
void initHistogram1024(void){
    ( hipMalloc((void **)&d_Result1024, HISTOGRAM_SIZE ));
}

//Internal memory deallocation
void closeHistogram1024(void){
    ( hipFree(d_Result1024) );
}
...
void histogram1024GPU(
    ...
    int dataN)
{
    ( hipMemset(d_Result1024, 0, HISTOGRAM_SIZE) );
    ...
    ( hipMemcpy(h_Result, d_Result1024, HISTOGRAM_SIZE, hipMemcpyDeviceToHost) );
}
