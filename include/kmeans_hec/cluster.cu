...
{    
  ...
  float *d_feature;
  hipMalloc((void**)&d_feature, npoints * nfeatures * sizeof(float));
  hipMemcpy(d_feature, features[0], npoints * nfeatures * sizeof(float), hipMemcpyHostToDevice);

  // d_feature_swap is written by the first kernel, and read by the second kernel 
  float *d_feature_swap;
  hipMalloc((void**)&d_feature_swap, npoints * nfeatures * sizeof(float));
  
  int *d_membership;
  hipMalloc((void**)&d_membership, npoints * sizeof(int));
  ...
  for(int nclusters = min_nclusters; nclusters <= max_nclusters; nclusters++)
  {
    ...
    hipLaunchKernelGGL(feature_transpose, grids, threads, 0, 0, d_feature_swap, d_feature, nfeatures, npoints);
    ...
    float* d_clusters;
    hipMalloc((void**)&d_clusters, nclusters * nfeatures * sizeof(float));
    ...
    for(int lp = 0; lp < nloops; lp++)
    {
      ...
      do {
        ...
        hipMemcpy(d_clusters, clusters[0], nclusters * nfeatures * sizeof(float), hipMemcpyHostToDevice);
        
        hipEventRecord(start,NULL);

        hipLaunchKernelGGL(find_membership, grids, threads, 0, 0, d_feature_swap, d_clusters, d_membership, nclusters, nfeatures, npoints);
        ...
        hipMemcpy(membership_OCL, d_membership, npoints * sizeof(int), hipMemcpyDeviceToHost);
