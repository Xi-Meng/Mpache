...
struct InjectPass_bypassL1 : public SubPass {
  ...
  void instrument(llvm::Instruction& inst) {   // bypass L1 cache L2
    if (check(inst)) {
      if (llvm::isa<llvm::LoadInst>(&inst)) {
  ...
  void runSubPass(llvm::Function& F) override {
    if (F.isDefinitionExact() && F.getCallingConv() == llvm::CallingConv::AMDGPU_KERNEL) {
        // if (F.getName()=="_Z19compute_step_factoriPfS_S_" || F.getName()=="_Z9time_stepiiPKfPfS0_S0_"){
          for (auto& BB : F) {
            for (auto& Inst : BB) {
              instrument(Inst);
            }
