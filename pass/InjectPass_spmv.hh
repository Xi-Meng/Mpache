...
struct InjectPass_spmv : public SubPass {
  ...
  bool check(llvm::Instruction& inst, llvm::raw_fd_ostream& file) {
    ...
    if (llvm::isa<llvm::LoadInst>(&inst)) {
      ...
      if (load_inst->getPointerAddressSpace() == 1 || load_inst->getPointerAddressSpace() == 5){
        ...
        file << result << "\n";
        // 1 2 4 5 7 10
        // if(result != "%26" && result != "%28" && result != "%38" && result != "%47" && result != "%61" && result != "%84"){
        // if(result != "%26" && result != "%28" && result != "%38" && result != "%60" && result != "%61" && result != "%84"){
        // if(result != "%84" && result != "%11" && result != "%21" && result != "%22" && result != "%34" && result != "%70" ){
        // if(result != "%26" && result != "%28" && result != "%38" && result != "%61" && result != "%66" && result != "%84" ){
        // !! 具有局部性：result != "%66"
        // 74的时候下降了4%-5%，其余影响都不是特别大
        // return false;
        if(result != "%84" && result != "%26" && result != "%28" && result != "%38" && result != "%61" && result != "%31" && result != "%47" && result != "%50" && result != "%74" && result != "%44")
        // if (result != "%41")
        {
