    ...
    if (F.isDefinitionExact() && F.getCallingConv() == llvm::CallingConv::AMDGPU_KERNEL) {
      for (auto& BB : F) {
        for (auto& Inst : BB) {
          ...
          // // 获取相关的信息:
          // llvm::Value* pointer = gep->getPointerOperand();
          // llvm::Value* index = gep->getOperand(1);
          // file << index << "\n";
          // Inst.dump();
          // file << "Inst.getName().str():" << Inst.getName().str() << "\n";
          // if (Inst.hasName() && (Inst.getName().str() == "20" || Inst.getName().str() == "38")) {
          //   file << "Found the value named %20 or %38!\n";
          //   // Calling your instrument function here
          //   // instrument(Inst);
          // }
