...
struct InjectPass_epistasis : public SubPass {
  ...
  bool check(llvm::Instruction& inst, llvm::raw_fd_ostream& file) {
    ...
    if (llvm::isa<llvm::LoadInst>(&inst)) {
      ...
      if (load_inst->getPointerAddressSpace() == 1 || load_inst->getPointerAddressSpace() == 5){
        ...
        file << result << " ";
        // load_require: 16
        // %56 %60 %64 %66 %110 %114 %118 %120 %171 %175 %179 %181 %225 %229 %233 %235
        // !! 具有局部性：
        // %56 %60 %64 %66 %110 %114 %118 %120 %171 %175 %179 %181 %225 %229 %233
        return false;
        if(result == "%235" ){
