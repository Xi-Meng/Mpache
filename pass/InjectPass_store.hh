...
struct InjectPass_store : public SubPass {
  ...
  bool check(llvm::Instruction& inst, llvm::raw_fd_ostream& file) {
    ...
    if (llvm::isa<llvm::LoadInst>(&inst)) {
      ...
      if (store_inst->getPointerAddressSpace() == 1 || store_inst->getPointerAddressSpace() == 5){
        ...
        store_search = std::getenv("STORE_IDX");
        int store_search_idx = 0;
        if (store_search != nullptr)  // 检查环境变量 "STORE_IDX" 是否存在
        {
          store_search_idx = std::stoi(store_search);  // 将环境变量 "STORE_IDX" 的值转换为 int，并赋值给 store_idx
        }
        if (store_search_idx == store_idx)
        {
          file << "bypass: " << store_idx << " : " << *store_inst <<"\n";
          return false;
        }
