...
struct InjectPass_ge_spmm : public SubPass {
  ...
  bool check(llvm::Instruction& inst, llvm::raw_fd_ostream& file) {
    ...
    if (llvm::isa<llvm::LoadInst>(&inst)) {
      ...
      if (load_inst->getPointerAddressSpace() == 1 || load_inst->getPointerAddressSpace() == 5){
        ...
        file << result << " ";
        // ge-spmm
        // %30 %34 %56 %58 %76 %95 %97 %116
        // spmm_my
        // %30 %39 %42 %47 %49 %57 %61 %72 %78 %86
        // 局部性 :78 > 72 > 57
        //%1: 57 72 %2: 30 86
        // return false; 
        // if(result =="%30" ||  result =="%39" ||  result =="%42" ||  result =="%47"  ||  result =="%49"  ||  result =="%61" ||  result =="%86" ){
        // if(result =="%30" ||  result =="%39" ||  result =="%42" ||  result =="%47"  ||  result =="%49"  ||  result =="%61" ||  result =="%86"){
        if(result =="%30" ||  result =="%86" ){
  ...
  void runSubPass(llvm::Function& F) override {
    ...
    if (F.isDefinitionExact() && F.getCallingConv() == llvm::CallingConv::AMDGPU_KERNEL) {
      ...
      // if (F.getName()=="_Z8spmm_jdsPfPKfPKiS3_S1_S3_ii"){
        for (auto& BB : F) {
          for (auto& Inst : BB) {
            // file << "Inst: " << Inst << "\n";
