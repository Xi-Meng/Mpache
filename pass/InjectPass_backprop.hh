...
struct InjectPass_backprop : public SubPass {
  ...
  bool check(llvm::Instruction& inst, llvm::raw_fd_ostream& file) {
    ...
    if (llvm::isa<llvm::LoadInst>(&inst)) {
      ...
      if (load_inst->getPointerAddressSpace() == 1 || load_inst->getPointerAddressSpace() == 5){
        ...
        // ---------------------------------
        // load_require: 12
        // %21 %26 %31 %36 %40 %43 %46 %54 %58 %63 %67 %70 
        // !! 具有局部性：%21 +0.09 %26 +0.018 %31 +0.01 %36 +0.014 %40 +0.007
        // %46 -0.012 %43 -0.009 
        // %43 单独会增加
        // --- 应该bypass单独的每一个：然后决定选择bypass哪几个
        // - or static：%46 -0.012 %54 %63 %67   
        // %58 %70
        // +: %21 %26 %31 %36 %40 %43 
        // return false;
        if(result == "%46" || result == "%54" || result == "%63" || result == "%67" || result == "%58" || result == "%70"){
  ...
  void runSubPass(llvm::Function& F) override {
    ...
    if (F.isDefinitionExact() && F.getCallingConv() == llvm::CallingConv::AMDGPU_KERNEL) {
      ...
      if (F.getName()=="_Z24bpnn_adjust_weights_cudaPfiS_iS_S_"){
        for (auto& BB : F) {
          for (auto& Inst : BB) {
            // file << "Inst: " << Inst << "\n";
