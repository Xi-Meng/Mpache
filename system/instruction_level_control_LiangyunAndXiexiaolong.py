...
'''
# 获取该APP的信息
app_name = "hybridsort_hip"
kernel_name = ["_Z12bucketprefixPjS_i"] # 考虑一个应用有多个kernel
...
HighThes3 = 0.7 # load theshold
pattern = "(.*) [mu]s"
iteration = 1 # 运行次数
log_name = "./system/liangyun_xiexiaolong/%s.txt" % app_name
...
def global_loads_bypass_algorithm(mode, groups, loads):
    ...
    for g in groups:
        ...
        _, offp, _ = offline_profiling(g, baseline_time)
    ...
    for i in range(len(groups)):
        ...
        #     continue
            ...
            for v in g:
                ...
                score = loads_map["%s" % v] + combi_g/len(g) # weight = 0
...
for kernel in kernel_name:
    ...
    policies = []
    scores = 0
    store_policies = False # bool lists
    for mode in modes:
    ...
    if (store_policies):
        ...
        os.environ['STOREBYPASS'] = "false"
    policy_time, offp, speedup = offline_profiling(policies, baseline_time)
    with open('%s' % log_name, 'a') as file:
        file.write("%s, store_bypass is %d:\n baseline_time:%f , time:%f, real improve:%f, speedup: %f \n" % ('_'.join(policies), store_policies, baseline_time, policy_time, offp, speedup))
