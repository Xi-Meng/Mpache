...
# 获取该APP的信息
app_name = "spmv_my"
kernel_name = ["_Z8spmv_jdsPfPKfPKiS3_S1_S3_i"] # 考虑一个应用有多个kernel
...
log_name = "./system/HighThes1/%s.txt" % app_name
...
def global_loads_bypass_algorithm(groups, loads):
    ...
    # total_score = 0.0
    # policy = []
    for HighThes1 in np.arange(1.6, 3, 0.1):
        print("HighThes1: %f ----------------------------" % HighThes1)
        with open('%s' % log_name, 'a') as file:
            file.write("HighThes1: %f -----------\n" % HighThes1)
        ...
        time, speedup, offp = offline_profiling(strSearch, strMode, storebypass, storemode, baseline_time)
        with open('%s' % log_name, 'a') as file:
            file.write("HighThes1:%f, %s, mode: %s,\n baseline_time:%f, design_time:%f, offp: %f, speedup: %f\n------\n" % (HighThes1, strSearch, strMode, baseline_time, time, offp, speedup))
